...
function TripsTable() {
    ...
    const handleEditStart = (trip: TripSummaryDto) => {
        ...
        Promise.all([getAllVehicles(), getAllProtocols()])
            ...
            .catch(err => setSaveError({ tripId: trip.id, message: err?.message || "Fahrzeuge/Protokolle konnten nicht geladen werden" }))
    }
    ...
    const handleEditSave = (trip: TripSummaryDto) => {
        ...
        updateTrip(trip.id, updatedTrip)
            ...
            .catch(err => setSaveError({ tripId: trip.id, message: err?.message || "Speichern fehlgeschlagen" }))
            ...
            <table className="ridesTable">
                <thead>
                    <tr>
                        ...
                        <th>Strecke</th>
                        <th>Protokoll</th>
                        {role === "PRIVAT" && (<><th colSpan={2}>Kilometerstand</th><th>Fahrbahnzustand</th></>)}
                        {role === "FAHRSCHÜLER" && (<><th colSpan={2}>Kilometerstand</th><th>Fahrbahnzustand</th></>)}
                        {role === "BERUFSFAHRER" && (<><th colSpan={2}>Kilometerstand</th><th>Tätigkeit</th><th>Fahrbahnzustand</th></>)}
                        <th>Aktionen</th>
                    </tr>
                    <tr>
                        ...
                        <>
                            ...
                            >
                                ...
                                <td onClick={e => e.stopPropagation()}>
                                    {isEditing(trip.id) ? (
                                        <select value={editValues!.vehicleId.toString()}
                                            ...
                                            {vehicles.map(v => <option key={v.id} value={v.id.toString()}>{v.model}</option>)}
                                        </select>
                                    ) : trip.vehicleModel}
                                </td>
                                ...
                                <td onClick={e => e.stopPropagation()} style={{ display: "flex", gap: "8px" }}>
                                    {isEditing(trip.id) ? (
                                        <>
                                            <Button label={saving ? "Speichern..." : "Speichern"} onClick={() => handleEditSave(trip)} stopPropagation />
                            ...
                            {/* Speicherfehler inline unter der bearbeiteten Zeile */}
                            {saveError?.tripId === trip.id && (
                                <tr key={`${trip.id}-error`}>
